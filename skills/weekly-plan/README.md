...
Shapes the week around what has to be true by Friday rather than around the calendar that filled itself. Protects one block for the work that has no deadline, because proactive work loses every collision it will ever have.
...
Works with nothing but your calendar and a sense of what is urgent. Gets better with a triage list, and an honest record of where last week went.
...
> Download the `weekly-plan` skill from https://github.com/CSPulse/customer-success-skills/tree/main/skills/weekly-plan, zip the `weekly-plan` folder on its own, then upload it as a skill in Claude.

Or do it by hand: download this repository as a ZIP (or clone it), zip this folder (`skills/weekly-plan`) on its own, then in Claude go to **Customize > Skills > Create skill > Upload a skill**. The folder name inside the ZIP has to match the `name` in `SKILL.md`.
