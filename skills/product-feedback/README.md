...
Turns customer noise into something product will act on: the job to be done rather than the requested feature, how many accounts share it, and what happens if nothing changes, said honestly. Closes the loop back, including on a no.
...
> Download the `product-feedback` skill from https://github.com/CSPulse/customer-success-skills/tree/main/skills/product-feedback, zip the `product-feedback` folder on its own, then upload it as a skill in Claude.
